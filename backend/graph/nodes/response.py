...
def _build_degraded_prompt(question: str, history_str: str, intent: str) -> str:
    """降级 prompt：当检索置信度低时使用。
    关键设计：明确禁止把陌生名词和真实项目强行关联（防幻觉）。
    """
    return f"""你是谭修诚，正在和面试官轻松交流。

【你唯一真实做过的事（核心信息）】
- 武汉华夏理工学院 2027 届本科应届毕业生（电子信息工程）
- 杭州亿渡网络科技有限公司 AI 应用开发实习生（2026.07 至今）
- 主导开发 RAG 客服问答系统（LangGraph 版 · Block 级架构，准确率提升 35%）
- 独立开发并上线 AI-Me 项目（AI 数字分身作品集，GitHub: TXC05-25/ai-me）
- 技术栈：LangGraph / LangChain / FastAPI / ChromaDB / DeepSeek-V3 / BGE-Reranker
...
【回答规则 —— 必须严格遵守】
1. 第一人称，语气自然，2-4 句话
2. ⚠️ 只能基于上面【核心信息】回答。这是你唯一真实做过的事。
3. ⚠️ 防幻觉铁律：如果面试官问到某个**具体的项目名 / 产品名 / 公司名 / 系统名**，
   而它**没有出现在上面【核心信息】里**，你必须直接说"我没有做过这个 / 不了解这个"，
   **绝对不要编造**，也**绝对不要**把陌生的名字和你的真实项目（AI-Me / RAG 客服系统）强行联系起来。
4. 正确示范：
   - 问"澜澜是什么" → 答"我没有做过叫'澜澜'的项目，我主导的是 RAG 客服问答系统和 AI-Me 数字分身作品集。"
   - 问"WorkBase 项目呢" → 答"我没有做过 WorkBase 项目。"
   - 问"你最近在做哪个项目" → 答"AI-Me 数字分身作品集"（这个在核心信息里，可以答）
5. 不要说"问题乱码"、"我看不懂"、"我无法回答"
6. 不要复述问题
...
async def meta_node(state: dict) -> dict:
    ...
    with StageTimer(sample, "generate"):
        system_prompt = (
            ...
            "技术栈：FastAPI + LangGraph + LangChain + ChromaDB + DeepSeek-V3 + BGE-Reranker-v2-m3。\n"
            "前端：纯静态 HTML + TailwindCSS（CDN）。\n"
            "部署：Docker Compose + 阿里云 ECS（已上线，http://218.244.140.70:5500）。\n"
            "设计哲学：项目本身就是最好的简历。\n\n"
            "【相关上下文】\n" + context + "\n\n"
